function dydt = ODE_SCG_CLOCK(t, y, par_clock, par_scg, t_V)
    ...
    dydt = zeros(n_E + n_I + n_clock + 6, 1);     % 0-12 for clock, and then for SCG
...
%         REV + kp_rev * Rev;
    ...
    if (t < t_V)
        y(n_clock+n_E+n_I+2) = 0;
        y(n_clock+n_E+n_I+3) = 0;
    end

    % T
    dydt(n_clock+1) = -1 * (1 + c_BMAL1_V * BMAL1 / (K_BMAL1_V + BMAL1)) * beta * y(n_clock+1) * y(n_clock+n_E+n_I+2);

    % ODEs for E_1,...,E_{n_E}
    dydt(n_clock+2) = (1 + c_BMAL1_V * BMAL1 / (K_BMAL1_V + BMAL1)) * beta * y(n_clock+1) * y(n_clock+n_E+n_I+2) - 1 * n_E / tau_E * y(n_clock+2);
    for i = (n_clock+3):(n_clock+n_E+1)
        dydt(i) = n_E / tau_E * y(i-1) - n_E / tau_E * y(i);
    end

    % ODEs for I_1,...,I_{n_I}
    dydt(n_clock+n_E+2) = n_E / tau_E * y(n_clock+n_E+1) - n_I / tau_I * y(n_clock+n_E+2);
    for j = (n_clock+n_E+3):(n_clock+n_E+n_I+1)
        dydt(j) = n_I / tau_I * y(j-1) - n_I / tau_I * y(j);
    end

     % V_inf, V_tot, IL6, IL1b, IL10
    for j = (n_clock+n_E+2):(n_clock+n_E+n_I+1)
        dydt(n_clock+n_E+n_I+2) = dydt(n_clock+n_E+n_I+2) + K_REV_V / (K_REV_V + REV) * p_inf * y(j);
    end
    dydt(n_clock+n_E+n_I+2) = dydt(n_clock+n_E+n_I+2) - c_inf * y(n_clock+n_E+n_I+2);
    
    for j = (n_clock+n_E+2):(n_clock+n_E+n_I+1)
        dydt(n_clock+n_E+n_I+3) = dydt(n_clock+n_E+n_I+3) + K_REV_V / (K_REV_V + REV) * p_tot * y(j);
    end
    dydt(n_clock+n_E+n_I+3) = dydt(n_clock+n_E+n_I+3) - c_tot * y(n_clock+n_E+n_I+3);

    for j = (n_clock+n_E+2):(n_clock+n_E+n_I+1)
        dydt(n_clock+n_E+n_I+4) = dydt(n_clock+n_E+n_I+4) + K_REV_IL6 / (K_REV_IL6 + REV) * p_IL6 * y(j);
    end
    dydt(n_clock+n_E+n_I+4) = dydt(n_clock+n_E+n_I+4) - d_IL6 * y(n_clock+n_E+n_I+4);

    for j = (n_clock+n_E+2):(n_clock+n_E+n_I+1)
        dydt(n_clock+n_E+n_I+5) = dydt(n_clock+n_E+n_I+5) + K_REV_IL1b / (K_REV_IL1b + REV) * p_IL1b * y(j);
    end
    dydt(n_clock+n_E+n_I+5) = dydt(n_clock+n_E+n_I+5) - d_IL1b * y(n_clock+n_E+n_I+5);

    for j = (n_clock+n_E+2):(n_clock+n_E+n_I+1)
        dydt(n_clock+n_E+n_I+6) = dydt(n_clock+n_E+n_I+6) + K_REV_IL10 / (K_REV_IL10 + REV) * p_IL10 * y(j);
    end
    dydt(n_clock+n_E+n_I+6) = dydt(n_clock+n_E+n_I+6) - d_IL10 * y(n_clock+n_E+n_I+6);
