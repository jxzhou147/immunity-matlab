function dydt = ODE_SCG_CLOCK(t, y, par_clock, par_scg, t_V)
    ...
    [tau_E, tau_I, n_E, n_I, p_inf, p_tot, c_inf, c_B, K_REV_V] = deal(par_scg{:});
    ...
    dydt = zeros(n_E + n_I + n_clock + 2, 1);     % 0-12 for clock, and then for Ei, Ij, V
...
%         REV + kp_rev * Rev;
    ...
    for j = (n_E+n_clock+1):(n_E+n_I+n_clock)
        dydt(n_E+n_I+n_clock+1) = dydt(n_E+n_I+n_clock+1) + K_REV_V / (K_REV_V + BMAL1) * p_inf * y(j);
    end
    dydt(n_E+n_I+n_clock+1) = dydt(n_E+n_I+n_clock+1) - c_inf * y(n_E+n_I+n_clock+1);

    % ODE for B
    for j = (n_E+n_clock+1):(n_E+n_I+n_clock)
        dydt(n_E+n_I+n_clock+2) = dydt(n_E+n_I+n_clock+2) + K_REV_V / (K_REV_V + BMAL1) * p_tot * y(j);
    end
