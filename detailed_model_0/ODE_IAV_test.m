function dydt = ODE_IAV_test(t, y, par)
    ...
    [beta, gamma, eta, alpha_MI, alpha_KI, alpha_TI, ...
        ...
        K_TI, d_I, d_M, d_K, d_T, clock] = deal(par{:});
    
    % variables
    y_cell = num2cell(y);
    [H, If, V, M, K, T, T_E] = deal(y_cell{:});
    
    c_clock = (sin(2 * pi * t / 24 + clock) * 0.5 + 1);
    %c_clock = clock;
    if (t > 6)
        c_clock = 1;
    end
    ...
    if V < 0
        ...
        dydt(3) = c_clock * gamma * If - alpha_MV * M * V;
    end
...
%     n_IM = 1;
    if M < 0
        ...
        dydt(4) = c_clock * c_IM  * FracNoInf(RealRootPromise(If, n_IM), (RealRootPromise(K_IM, n_IM) + RealRootPromise(If, n_IM))) ...
            + c_M * FracNoInf(M, (K_M + M)) - 0.5 * d_M * M;
%         dydt(4) = c_clock * 1e6 / (1e6 + M) * c_IM * FracNoInf(RealRootPromise(If, n_IM), (RealRootPromise(K_IM, n_IM) + RealRootPromise(If, n_IM))) ...
