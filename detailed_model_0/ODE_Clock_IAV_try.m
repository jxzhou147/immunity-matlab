function dydt = ODE_Clock_IAV_try(t, y, par_clock, par_IAV, t_IAV)
    ...
    if (V < 0) || (t < t_IAV)
        ...
        dydt(15) = 2 * FracNoInf(K_B, K_B + BMAL1) * gamma * If - alpha_MV * M * V;
    end
...
%         if t < (t_IAV + 8)
%             dydt(16) = 0.5 * FracNoInf(2, 0.1 + BMAL1) * c_IM * ...
%                 FracNoInf(RealRootPromise(If, n_IM), (RealRootPromise(K_IM, n_IM) + RealRootPromise(If, n_IM))) ...
%                 + c_M * FracNoInf(M, (K_M + M)) - 0.8 * d_M * M;
%         else
%             dydt(16) = 0.2 * c_IM * ...
%                 FracNoInf(RealRootPromise(If, n_IM), (RealRootPromise(K_IM, n_IM) + RealRootPromise(If, n_IM))) ...
%                 + c_M * FracNoInf(M .^ 3, (K_M .^ 3 + M .^ 3)) - 0.8 * d_M * M;
%         end
        dydt(16) = 0.5 *  FracNoInf(K_B, K_B + BMAL1) * c_IM * ...
            FracNoInf(RealRootPromise(If, n_IM), (RealRootPromise(K_IM, n_IM) + RealRootPromise(If, n_IM))) ...
            + 100 * FracNoInf(K_B, K_B + BMAL1) * c_M * FracNoInf(M, (K_M + M)) - 0.3 * d_M * M;
    end
