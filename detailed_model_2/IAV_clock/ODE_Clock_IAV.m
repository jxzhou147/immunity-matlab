function dydt = ODE_Clock_IAV(t, y, par_clock, par_IAV, t_IAV)
    ...
    [beta, gamma, eta, ...
        ...
        K_REV_CCL2, K_REV_CXCL5, c_ROR_IL6, K_ROR_IL6, c_IL6_REV, K_IL6_REV, n_IL6_REV] ...
        = deal(par_IAV{:});
...
%         REV + kp_rev * Rev;
    ...
    %     dydt(1) = - beta * H * V;
        ...
        dydt(14) = beta * H * V - FracNoInf(K_B_M, (K_B_M + BMAL1)) * (1 + c_IL6_M * FracNoInf(IL6, (K_IL6_M + IL6))) * a_MI * FracNoInf((M + Mono), (K_MI + M + Mono)) * If - ...
            a_NI * N * If - (1 + FracNoInf(c_P_K * PER, (K_P_K + PER))) * a_KI * K * If - a_TI * T_E * If - d_I * If;

        dydt(15) = FracNoInf(K_B_M, (K_B_M + BMAL1)) * (1 + c_IL6_M * FracNoInf(IL6, (K_IL6_M + IL6))) * a_MI * FracNoInf((M + Mono), (K_MI + M + Mono)) * If + ...
            a_NI * N * If + (1 + FracNoInf(c_P_K * PER, (K_P_K + PER))) * a_KI * K * If + a_TI * T_E * If + d_I * If -a_MD * (M + Mono) * D;

        dydt(16) = FracNoInf(K_REV_V, (K_REV_V + REV)) * gamma * If - a_NV * N * V - d_V * V;

        dydt(17) = b_M * d_M - c_IM * M * If - d_M * M;

    %     c_CCL2_Mono = (1 - exp(-0.1 * t)) * c_CCL2_Mono;
        dydt(18) = c_CCL2_Mono * (1 + c_IL6_Mono * FracNoInf(IL6, (K_IL6_Mono + IL6))) * CCL2 * (Mono_tot - Mono) - d_Mono * Mono;

        dydt(19) = (c_N + c_CXCL5_N * (1 + c_IL6_N * FracNoInf(IL6, (K_IL6_N + IL6))) * CXCL5) * ((c_N + d_N) / c_N * b_N - N) - d_N * N;

        dydt(20) = FracNoInf(K_BMAL1_IL6, K_BMAL1_IL6 + BMAL1) * FracNoInf(K_REV_IL6, K_REV_IL6 +REV) * ...
            (1 + c_ROR_IL6 * FracNoInf(ROR, K_ROR_IL6 + ROR)) * FracNoInf(K_IL10_IL6, K_IL10_IL6 + IL10) * c_M_IL6 * (M + Mono) + ...
...
%             end
    ...
    %     c_M_CCL2 = (1 - exp(-0.1 * t)) * c_M_CCL2;
        ...
        dydt(24) = (1 + c_IL6_K * FracNoInf(IL6, (K_IL6_K + IL6))) * (c_K + c_MK * (M + Mono) + c_IK * If) * ((c_K + d_K) / c_K * b_K - K) - d_K * K;
    ...
    %     dydt(14) = c_MT * FracNoInf(RealRootPromise((M + Mono), n_MT), (RealRootPromise(K_MT, n_MT) + RealRootPromise((M + Mono), n_MT))) * T - d_T * T_E;
        ...
        dydt(26) = c_MT * FracNoInf(RealRootPromise(K, n_MT), (RealRootPromise(K_MT, n_MT) + RealRootPromise(K, n_MT))) * T - d_T * T_E;
    end
end