function dydt = ODE_Clock_IAV(t, y, par_clock, par_IAV, t_IAV, par_infla)
    ...
    dydt = zeros(28, 1);    % y(28): weight change
...
%     dydt(8) = -1 * dp_rev * REV + kp_rev * Rev;
    ...
    if (t >= t_IAV)
        ...
        dydt(14) = beta * H * V - FracNoInf(K_B_M, (K_B_M + BMAL1)) * a_MI * FracNoInf((M + Mono), (K_MI + M + Mono)) * If - ...
            a_NI * N * If - (1 + FracNoInf(c_P_K * PER, (K_P_K + PER))) * a_KI  * FracNoInf(70 ^ 5, 70 ^ 5 + RealRootPromise(M + Mono, 5)) * K * If - a_TI * FracNoInf(70 ^ 5, 70 ^ 5 + RealRootPromise(M + Mono, 5)) *  T_E * If * 200 / (200 + If) - d_I * If;
%         dydt(14) = beta * H * V - 5 * (FracNoInf(K_B_M, (K_B_M + BMAL1)) * a_MI * FracNoInf(M + Mono, M + Mono + N + K + T_E) * FracNoInf((M + Mono), (K_MI + M + Mono)) * If + ...
%             a_NI * FracNoInf(N, M + Mono + N + K + T_E) * N * If + (1 + FracNoInf(c_P_K * PER, (K_P_K + PER))) * a_KI * FracNoInf(K, M + Mono + N + K + T_E) * K * If + a_TI * FracNoInf(T_E, M + Mono + N + K + T_E) * T_E * If) - d_I * If;
...
%         dydt(19) = c_CCL2_Mono * (1 + c_IL1b_Mono * FracNoInf(IL1b, (K_IL1b_Mono + IL1b))) * 1000 * FracNoInf(CCL2 .^ 3, CCL2 .^ 3 + 2000 .^ 3) * (Mono_tot - Mono) - d_Mono * Mono;
        dydt(19) = c_CCL2_Mono * (1 + c_IL1b_Mono * FracNoInf(IL1b, (K_IL1b_Mono + IL1b))) * 1000 * FracNoInf(CCL2, CCL2 + 500) * (Mono_tot - Mono) - d_Mono * Mono;
...
%             (1 + c_ROR_IL1b * FracNoInf(ROR, K_ROR_IL1b + ROR)) * FracNoInf(K_IL10_IL1b, K_IL10_IL1b + IL10) * c_M_IL1b * (M + Mono) + FracNoInf(K_REV_IL1b, K_REV_IL1b +REV) * c_I_IL1b * (If + D)) - d_IL1b * IL1b;
        dydt(21) = (FracNoInf(K_BMAL1_IL1b, K_BMAL1_IL1b + BMAL1) * FracNoInf(K_REV_IL1b, K_REV_IL1b +REV) * ...
            (1 + c_ROR_IL1b * FracNoInf(ROR, K_ROR_IL1b + ROR)) * FracNoInf(K_IL10_IL1b, K_IL10_IL1b + IL10) * c_M_IL1b * (M + Mono) + c_I_IL1b * If) * ...
        (1 + c_D_IL1b * FracNoInf(RealRootPromise(D, n_D_IL1b), RealRootPromise(K_D_IL1b, n_D_IL1b) + RealRootPromise(D, n_D_IL1b))) - d_IL1b * IL1b;
...
%         dydt(25) = (1 + c_IL1b_K * FracNoInf(IL1b, (K_IL1b_K + IL1b))) * (c_K + c_MK * M + c_IK * If) * ((c_K + d_K) / c_K * b_K - K) - d_K * K;
        dydt(25) = (1 + c_IL1b_K * FracNoInf(IL1b, (K_IL1b_K + IL1b))) * (c_K + c_MK * M + c_IK * 20 * FracNoInf(If, 10 + If)) * ((c_K + d_K) / c_K * b_K - K) - d_K * K;
    ...
    %     infla = sigma_M * M + sigma_Mono * Mono + sigma_N * N;
        ...
        dydt(28) = sigma_I * FracNoInf(RealRootPromise(If, n_I), RealRootPromise(K_I, n_I) + RealRootPromise(If, n_I)) + ...
